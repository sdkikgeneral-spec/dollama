# -*- coding: utf-8 -*-
"""施策C seed sweep 集計・判定。

_results/eval_persample_{base,d5}_{seed}.npz と eval_report_{...}.json を読み、
seed x {#1,D5} x {diverse_a,diverse_b} の macro F1/Jaccard 表、各 seed の paired
delta (D5-#1)、across-seed 平均±sd、paired bootstrap CI、paired t を出す。

判定軸 (タスク 4):
  (a) 全 seed で delta の符号が + で一貫するか
  (b) delta 平均が #1 自身の seed 分散帯 (diverse 用に #1 を seed 間で比べた band) を超えるか
  (c) 各 seed の paired bootstrap 95%CI が 0 を含まないか

per-sample 配列は rows と同順・NaN=skip/未定義。paired 解析は両 arm とも非 NaN の
位置のみ採用 (#1/D5 で skip 集合は同一なので落ちる位置はほぼ同じ)。
"""
...
RES = os.path.join(ROOT, "data", "bitnet", "_seedsweep", "_results")
...
def paired_arrays(base_npz, d5_npz, s, metric):
    """両 arm で非 NaN の位置のみ揃えた (base, d5) 配列を返す。"""
    b = base_npz[f"{s}__{metric}"].astype(float)
    d = d5_npz[f"{s}__{metric}"].astype(float)
...
def paired_t(b, d):
    ...
    t = m / (sd / sqrt(n))
    # 両側 p (t 分布・自前 survival 近似回避のため scipy があれば使う)
    try:
        from scipy import stats
        p = 2 * stats.t.sf(abs(t), df=n - 1)
    except Exception:
        # scipy 無ければ正規近似 (n=~1500 で十分妥当)
...
def main():
    ...
    for arm in ("base", "d5"):
        for seed in SEEDS:
            ...
            for s in SETS:
                ...
                # report の macro と突合 (一致しないと per-sample 定義ズレ)
                if s in macro:
                    for m in METRICS:
    ...
    print("=" * 92)
    print("施策C seed sweep: seed x {#1(base),D5} x {diverse_a,diverse_b} macro 表")
    print("=" * 92)
    ...
    print("\n" + "=" * 92)
    print("per-seed paired delta = D5 - #1 (同 seed・per-sample paired)")
    print("=" * 92)
    deltas = {(s, m): [] for s in SETS for m in METRICS}  # across-seed delta means
    ci_contains_zero = {(s, m): [] for s in SETS for m in METRICS}
    sign_consistent = {(s, m): [] for s in SETS for m in METRICS}
    for s in SETS:
        for m in METRICS:
            print(f"\n--- {s} / {m} ---")
            print(f"{'seed':>9} {'#1':>8} {'D5':>8} {'delta':>9} "
                  f"{'CI_lo':>9} {'CI_hi':>9} {'t':>8} {'p':>10} {'n_pair':>7}")
            for seed in SEEDS:
                bn = load_persample("base", seed)
                dn = load_persample("d5", seed)
    ...
    print("=" * 92)
    # #1 自身の diverse seed 分散帯 (base の各 seed macro を seed 間で比較)
    base_band = {}
    for s in SETS:
        for m in METRICS:
    ...
    print("\n" + "=" * 92)
    print("結論サマリ: 上の (a)+(b)+(c) を全 set/metric で満たせば D5 diverse 優位は seed 頑健。")
    print("=" * 92)
