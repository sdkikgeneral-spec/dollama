...
| 自作 VAE decoder 全段 | SSIM **0.999988** (旧掲載 0.999992 は **2-6 最適化の中で 0.999992 → 0.999988 に変化**していたもので、本表が追随していなかった分の訂正。**変化点は S2 (`b3fe139`・2026-06-20 = conv を im2col+wmma GEMM 化 + 帯分割 stride バグ修正) で、この時点で既に 0.999988** (同 commit 本文「noise_pred SSIM 0.999996 / VAE SSIM 0.999988 復帰」)。以降の S3-A (`502c936`) / S3-D (`2e079e1`) / S3-E (`a71898e`) は本文がいずれも 0.999988 を**維持**と記録しており、**S3-E を変化点として名指ししないこと** (S5b の記述は誤り。犯人を取り違えると VAE 精度を疑う担当が TF32 化を revert しに行き、真の変化点 S2 の FP16 im2col+wmma conv を見逃す)。出典 commit `b3fe139` / `a71898e`。0.999992 は 2-4 実装当時の正しい実測値であり誤記ではない。**G-8k S3/S3c は VAE 数値を動かしていない** — G-8k S3 前後で出力 FP16 画像が memcmp BIT-EXACT (出典 commit `88b3ae7` 本文)) / decode 7.96s | test_vae_decode |
