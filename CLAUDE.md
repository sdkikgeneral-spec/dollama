...
- 拡散重み: 既定 preset = `illustrious-xl`・素 SDXL は `--preset base` 明示 (詳細は計測表「アニメ特化 SDXL 3 preset (2-6d)」行)
...
> ★セル中の「measurements-log ★退避節「〜」」= `docs/measurements-log.md` の
> **「CLAUDE.md 計測表セルの全文退避 (2026-09-20・CLAUDE.md 肥大化是正)」節**の同名小節。
> 圧縮前のセル全文 (経緯・教訓・characterization 値) をそのまま保持しており、**そちらが当該セルの正本**。
...
| 自作 VAE decoder 全段 | SSIM **0.999988** (旧掲載 0.999992 からの変化点は `b3fe139` 単独。★**帰属を取り違えた是正が 2 回続いた箇所** — 経緯・出典・禁止事項は measurements-log 「G-8k S5b/S5c/S5d/S5e 残債」①) / ~~decode 7.96s~~ ★**2-6 最適化に未追随の陳腐化値・要再測** (同 残債④。**見積り・合否の分母に使わないこと**) | test_vae_decode |
| 自作 SDXL UNet 全段 | noise_pred SSIM **0.999998** (★2-5 当時の値。2-6 後の実測は **0.999996** = 同 残債③) / ~~1step ~9.2s~~ ★**陳腐化・要再測** (同 残債④) / 2.57B params | test_unet |
| UNet バッチ (G-2k S2) | `launch_unet_batched(B=2)` per-sample パリティ **実走緑** (SAC OFF): MAE **6.4e-05**/bad0。ビット一致は非到達 = FP16 tol 内 (1 ULP)・conv2d S1 は per-n ループで **BIT-EXACT**。詳細は measurements-log ★退避節「UNet バッチ (G-2k S2)」 | test_unet/conv2d |
| CFG batch2 パリティ+速度 (G-2k S3c **完了**) | **parity gate g=1.0 SSIM 0.9994≥0.999 PASS** (SAC OFF 実走)・guidance sweep g=1/3/7.5 = 0.9995/0.9988/0.9966 (FP16 tiling が CFG で単調増幅)。**正典 CFG e2e (20step/warm/VAE 込)**: default **20.93s** / batch2 単独 **17.59s(1.19x)** / **--fast(attn+batch2) 15.88s(1.32x)** ★**この 3 値は 2026-07-09 = pre-G-8k の実測で陳腐化・要再測。見積り・合否の分母に使わないこと** (再測の担い手は未定)。経緯・理由は measurements-log ★退避節「CFG batch2 パリティ+速度 (G-2k S3c 完了)」 | test_diffusion_batch2 |
...
| GroupNorm multi-block (G-4k S1a 完了) | `launch_group_norm_mb` 2段決定的集約 (atomic 禁止): 帯域 unet_320_128 **444.6 GB/s** (1-block 105 → **4.2x**)・parity MAE~3e-7・bitexact 3runs 一致。epilogue vs default **SSIM 0.999999**/bad0・default 無改変維持。詳細は measurements-log ★退避節「GroupNorm multi-block (G-4k S1a 完了)」 | test_groupnorm/test_unet_fast |
| GroupNorm+SiLU 融合 (G-4k S1b 完了) | `launch_group_norm_silu` = mb normalize 末尾に SiLU を融合・resnet epilogue の norm1/2 が 2 パス→1 パス。**5 形状すべて BIT-EXACT vs mb+silu** (memcmp)。UNet: epilogue vs default **SSIM 0.999999**/MAE6.6e-5/bad0・default 無改変維持。詳細は measurements-log ★退避節「GroupNorm+SiLU 融合 (G-4k S1b 完了)」 | test_groupnorm/test_unet_fast |
| conv2d 後段融合 (G-4k S2 完了) | resnet epilogue の conv 後段を融合 2 本に (`launch_conv_bias_biasch` / `launch_conv_bias_residual`)・**全 6 形状 (B1/2×320_128/640_64/1280_32) BIT-EXACT** (memcmp)・非保証 shape は `conv2d_uses_gemm_bias_path` ガードで従来 2 パスへフォールバック。UNet: epilogue vs default **SSIM 0.999999**/MAE6.6e-5/bad0・default 無改変維持。**resnet ≤0.95s 再 profile (2026-07-14 実走)=不合格**: default 1.226s→fast+epi **1.212s (−1.1%=ノイズ床内)**・バケットは conv2d 質量で GN 4.2x は非寄与。★**ゲートは今も未達** (G-10k 陰性クローズ後も) ・★**B=1 計器 `prof_unet_fast_warm` で再 profile しないこと** (batch2 構成の resnet を測れない = G-10k T8 の計器誤り指摘・正本 `docs/measurements-log.md` G-10k 節 規律 1)。急所 (丸め列の差) ・再割当の全経緯は measurements-log ★退避節「conv2d 後段融合 (G-4k S2 完了)」 | test_bias_add/test_unet_fast/prof_unet_fast_warm |
| **epilogue 出荷結線 (G-4k S3 完了 = G-4k クローズ)** | `DOLLAMA_EPILOGUE` env を `FastConfig` に追加 (`--fast` が含意) + `DiffusionPipeline` へ貫通 → **出荷 `--fast` = attn_fast+batch2+epilogue**。**実走 (SAC OFF・全 PASS)**: GATE1 (ノイズ床) / GATE3 (epilogue determinism) **bit-exact** / GATE2 batch2 vs off SSIM **0.999474** / GATE4 **(fast+epi) vs fast @g=1.0 SSIM 0.999477** (≥0.999)。**教訓: CFG 増幅下 (g>1) で FP16 微差をゲートしない。被験変数は g=1.0 で分離する** (初版の g=7.5 ゲートは 0.996533 = 正常でも FAIL する実証あり)。**e2e DB2_BENCH 20step warm**: default 24662.8ms / **fast+epilogue (出荷 --fast) 18482.4ms x1.33439** ★**pre-G-8k (2026-07-28) 実測で陳腐化・要再測。分母に使わないこと** (再測の担い手は未定)・★**本走行は熱で全体 ~18% ドリフト → 相対倍率 x1.33 が主指標・絶対秒は条件付き**。ゲート再設計の全経緯・characterization 値は measurements-log ★退避節「epilogue 出荷結線 (G-4k S3 完了 = G-4k クローズ)」 | test_diffusion_batch2/test_unet_fast/prof_unet_fast_warm |
| **device_arena e2e VRAM 収支 (G-8k S1〜S4b 完了 = G-8k クローズ / S6 レビュー是正済)** | UNet forward・VAE decode・conv2d im2col の一時バッファをチャンク連結 bump アリーナ `device_arena` + 事前 reserve 化 (既定 ON・キルスイッチ `DOLLAMA_POOL=0`)。S4b e2e (研究機・SAC OFF) **6 ハードゲート全 PASS** (うち芯 4 本を再掲・全 6 本 = G0/G1/G2'/G4/G5'/G6 は退避節) = S4 の G4 FAIL (peak 16302MB 物理張り付き) を S3b/S3c で解消: 出力 18 枚 bit 一致 / step ループ内 実 cudaMalloc/cudaFree **0** / peak **13649MB ≤ 基準 POOL=0 13269+512MB** / 秒 +5% 以内。トレードオフ = 定常 residency +6.5GB。**S6 (レビュー是正 2026-08-22)**: **最終ツリー `meson test` 53/53 緑・出力 sha256 全一致で数値不変**・VRAM **delta +340MB**。★**判定は同一セッションの POOL=0 比 delta** (絶対値は同居量で平行移動)・★**F4/F2 の異常系は未発火 =「実走で確認した」と書かない**・★**「秒数レバーではない」は起票目的の話で「実測で秒が動かなかった」の意ではない** (`docs/fast-mode-plan.md`「用語の解消」節)。詳細・経緯の正本は `docs/measurements-log.md` G-8k S4/S4b/**S6** 行と「G-8k S6 (T2)」節・★退避節「device_arena e2e VRAM 収支 (G-8k S1〜S4b 完了 = G-8k クローズ / S6 レビュー是正済)」 + `docs/fast-mode-plan.md` G-8k 実装記録 | test_device_arena/test_http (`prof_arena_e2e` は計測専用・meson test 未登録) |
| **conv2d 真 batch2 (G-10k・陰性クローズ 2026-09-17)** | 数値ゲート全緑 ([G1] bit-exact / GATE2 MAE 0.0303612・SSIM 0.999474・GATE4 0.999467) だが fast+epilogue resnet バケット削減率 **+3.38% / +1.87% (悪化・分母 = 同セット `DOLLAMA_CONV_BATCH=0`)** = 秒中立 → **opt-in `DOLLAMA_CONV_BATCH=1` に降格・既定は per-n 直列** (`c3ee3ca`)。律速 = GEMM/im2col が N に比例。経緯・規律 (分母退役・計装 ON 秒の扱い) は `docs/measurements-log.md` G-10k 節 / ★退避節「conv2d 真 batch2 (G-10k・陰性クローズ 2026-09-17)」 / `docs/g10k-plan.md` §13 | test_gemm/test_conv2d (+`conv2d_batch_on`)/test_diffusion_batch2 |
...
| ランタイム LoRA (L-2 完了) | kohya→diffusers 写像 (`load_lora_modules`) + 常駐重み apply-time マージ (`unet_apply_loras`) / `unet_clear_loras` bit-exact 復元。**全 4 ゲート PASS** (SAC OFF 実走・うち芯 3 本を再掲・[1]host 写像を含む全数は退避節): parity max_abs **4.9e-4**/bad0・revert 4/4 memcmp bit-exact・stack max_abs **9.6e-4**/bad0+revert bit-exact。数値正典 = L-1 offline merge (dollma_merge_lora.py) `W+strength*(alpha/rank)*(B@A)`。HTTP `loras:[{name,strength}]` 結線 (name は allowlist 検証で path traversal 封止)。**⚠ 後日判明した ODR 違反** (unet.cu / vae_decode.cu の同名・異レイアウト `class DeviceWeights`・L-2 の `patched_` が顕在化 → `~DiffusionPipeline` で **0xC0000005**) は **C0 で匿名 namespace 化して解消**。**教訓: 単体ハンドルの test は、複数ハンドル同居時の破棄経路を検査しない** (`test_lora_runtime` は素通し・`test_diffusion_batch2` で初めて露見)。全経緯は measurements-log ★退避節「ランタイム LoRA (L-2 完了)」 | test_lora_runtime (+回帰ゲート: test_diffusion_batch2) |
...
| 指標 | 値 | test |
|---|---|---|
| 自作フル拡散 (2-6a) | 20step 1024² 84.07s → **11.30s** (2-6 最適化後・律速 UNet attn 4.60s)。★**採取条件 = 非 CFG (guidance=1.0)・B=1・`generate()` 経路・2026-06-22 = 出荷 txt2img (`generate_txt2img` = CFG・B=2) の数字ではない**。**pre-G-2k/3k/4k/8k で陳腐化・要再測** (一次証拠と禁止事項は measurements-log 計測表の 2-6a 行) | test_diffusion |
...
| アニメ特化 SDXL 3 preset (2-6d) | `models/presets/{noobai-xl,animagine-xl-4,illustrious-xl}/` = unet/vae/TE-L/TE-G 4 ファイル差し替え・**既定 illustrious-xl** (`--preset base` で素 SDXL)。照合済 (UNet 1680 キー名+shape/dtype・tokenizer sha256 が base 一致・TE 変換誤差 1e-5 台 ×3 preset)。12 枚比較 (seed 1234・**N=1**) のユーザー目視順位 illustrious > noobai > animagine。✅ 2-6e prefix 自動付与 (`--no-preset-prefix` / HTTP `preset_prefix:false` で OFF)。未着手: HTTP API preset / UI 選択。照合の内訳・12 枚比較の条件・残債 4 件 (illustrious VAE scaling_factor 0.18215 の正しい値は未確定 等) は measurements-log 「2-6d」節 | test_preset |
...
4. **記録 (docs / CLAUDE.md 計測表 / commit 本文) の執筆・是正は `record-writer`、その検査は `record-auditor` に分ける。** 同一主体に書かせて検査させない (G-8k S5 で是正が新しい誤りを持ち込む形が 5 ラウンド連続で起き、毎回この分離が検出した)。
5. **コンポーネントを実装したら、必ずテストも実装する。** `src/tests/test_<component>.cpp` を作成し、`meson test -C build` が通ることを確認してから完了とする。テスト規約は `docs/testing.md` 参照。
