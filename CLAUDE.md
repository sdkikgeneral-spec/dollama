...
| **conv2d 真 batch2 (G-10k・陰性クローズ 2026-09-17)** | 数値ゲート全緑 ([G1] bit-exact / GATE2 SSIM 0.999474) だが fast+epilogue resnet バケット削減率 **+3.38% / +1.87% (悪化・分母 = 同セット `DOLLAMA_CONV_BATCH=0`)** = 秒中立 → **opt-in `DOLLAMA_CONV_BATCH=1` に降格・既定は per-n 直列** (`c3ee3ca`)。律速 = GEMM/im2col が N に比例。★B=1 の 1.212s は分母から退役・`DOLLAMA_PROFILE=1` の秒は正典でない。経緯・規律は `docs/measurements-log.md` G-10k 節 / `docs/g10k-plan.md` §13 | test_gemm/test_conv2d (+`conv2d_batch_on`)/test_diffusion_batch2 |
