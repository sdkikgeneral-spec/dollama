...
| 直近生成のミニ履歴 | プレビュー枠の下に**直近 4 枚**を新しい順で並べ、**クリックでその絵に戻る** (選択中は accent の枠)。戻ると**画像・モードバッジ・PNG 保存のファイル名・プリセットのサムネ元が 4 つ同時に**その生成のものになる (`Generate.razor` の `ShowResult` が唯一の合流点)。**セッション内メモリのみで永続化しない** — タブを閉じる/再接続すると消える。**削除・並べ替え・ピン留め・サムネごとの保存導線は無い** (「設定ごと復元」はプリセットの役割で、ここは「さっきの絵に戻る」だけ)。並び替え規則は `Services/PreviewHistory.cs`・生成中はクリックできない |
