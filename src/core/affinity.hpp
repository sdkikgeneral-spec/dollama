...
// スレッド/同期は STL (std::jthread) で書き、OS 依存はこのファイルだけに閉じ込める。
...
// 指定スレッドに CPU アフィニティマスクを設定する。
// t:    対象スレッド (std::jthread)。native_handle が有効である必要がある。
// mask: 論理プロセッサのビットマスク (bit i = 論理コア i を許可)。
// 戻り値: 成功で true、失敗 (mask=0 / API 失敗 / 非対応 OS) で false。
inline bool set_thread_affinity(std::jthread& t, uint64_t mask)
{
    ...
    DWORD_PTR prev = SetThreadAffinityMask(
        t.native_handle(), static_cast<DWORD_PTR>(mask));
    return prev != 0;
#elif defined(__linux__)
    cpu_set_t cs;
    CPU_ZERO(&cs);
