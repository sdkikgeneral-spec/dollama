...
| **CPU 側 LM 推論の速度最適化** | これまで速度詰めは CUDA に集中し、**CPU の LM 推論 [src/infer/bitnet.hpp](../src/infer/bitnet.hpp) は naive 参照のまま**（積和は全部 `double` 蓄積・SIMD/マルチスレッドなし・`lm_head` 4999×512 を毎回全走査）。seq8 ~253ms（GPU 版で 46.8x 済だが CPU 経路自体は未最適化）。CUDA に相当する CPU 側作業 = **AVX2/AVX-VNNI による int8 GEMM・matmul のマルチスレッド化・golden 経路外で double→float 化・lm_head 全走査の削減・BitLinear の毎回再量子化を排除**。素直に効く余地が大きく数倍見込み。**ただし急がない**: CPU LM は「拡散の裏で先行生成するフォールバック役」で、naive でも拡散の壁時間 (~3.8s) より十分速く実用上は足りている（2-6 で CUDA 速度詰めを打ち切ったのと同じ温度感）。**着手するなら先に軽くプロファイルして律速 (おそらく lm_head 全走査) を実測**してから。golden (`double` 蓄積) は突合用の別経路として保持。 | ◯ (全 HW 使い切る芯との整合では妥当・実用上は非緊急・時期未定) |
