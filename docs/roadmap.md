...
| 2-6d | **実 checkpoint 差し替え = アニメ特化 SDXL 3 preset** (NoobAI-XL 1.1 / Animagine XL 4.0 / Illustrious XL v0.1) | `server/preset.hpp` (preset→4 パス解決), `cli_generate.hpp` (`--preset` / env `DOLLAMA_BACKEND_PRESET`), `scripts/dollma_export_sdxl_preset.py` (変換), `THIRD_PARTY_NOTICES.md` | ✅ (2026-09-17・`7ccd6b7`〜`d8a9678`)。`models/presets/<name>/` の **unet/vae/text-encoder-l/g 4 ファイル**を差し替え・`SDXLBackend` 無改修。**既定 preset = illustrious-xl** (12 枚比較のユーザー目視 illustrious > noobai > animagine・`d8a9678`)・素 SDXL は `--preset base` 明示。変換は 3 preset とも epsilon・UNet 1680 キー名+shape 一致・tokenizer sha256 base 一致・TE 変換誤差 1e-5 台。**急所: 計画の「unet/vae 変換」では足りず TE-L/G も preset ごとに差し替える** (3 preset の TE OV bin sha256 は base と全て不一致 = checkpoint 固有)。preset.json の `prompt_prefix`/`negative_prefix` は **C++ 側で自動付与しない** (12 枚比較は CLI 引数で手動連結)。数値・照合・残債 (illustrious の VAE scaling_factor 0.18215 問題) は `docs/measurements-log.md` 「2-6d」節。**後続 (スコープ外・未着手)**: prompt_prefix の C++ 自動付与 / HTTP API の preset フィールド / UI の preset 選択 |
