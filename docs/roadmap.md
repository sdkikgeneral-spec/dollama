...
| 2-6b | 本 txt2img: dual encoder (CLIP-L+G) + CFG + prompt→embeds 結線 | `infer/{clip_tokenizer,clip_encoder2,text_conditioner}.hpp`, `server/diffusion_runner.*`, `server/txt2img_generator.hpp` | ✅ (c3b1dac)・NPU L/bigG・guidance 7.5・20step・PNG 1024²・test 35/35 緑。**prompt 供給元は将来 Phase 4 A の自作 LM に差し替える予定** (現状は `req.prompt` 直入力 = LM 段未結線) |
