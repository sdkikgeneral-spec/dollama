...
| # | 実装物 | ファイル | 状態 |
|---|---|---|---|
| 0 | Meson ビルド + `src/` ディレクトリ構造 | `meson.build`, `src/` | ✅ |
...
| 2-6 最適化 | direct conv→im2col/wmma・naive attn→flash | `unet.cu`/`vae_decode.cu`/`conv2d.cu`/`attention.cu` | 🟡 一旦クローズ **84s→11.30s** (7.44x)。残律速=UNet attn 4.60s (ライブラリ余地で保留)。本丸は Phase 4 へ。★**11.30s は 非CFG・B=1・`generate()` 経路・2026-06-22 = 出荷 txt2img の値ではない / pre-G-2k〜G-8k で陳腐化** (`docs/measurements-log.md` 2-6a 計測行) |
| 2-6b | 本 txt2img: dual encoder (CLIP-L+G) + CFG + prompt→embeds 結線 (純 cpp interface **`IDiffusionRunner`** で OV/CUDA を隔離) | `infer/{clip_tokenizer,clip_encoder2,text_conditioner}.hpp`, `server/diffusion_runner.*` (`IDiffusionRunner` 宣言 = `.hpp` / 実体 = `.cu` / OV・CUDA 無効ビルド用 `_stub.cpp`), `server/txt2img_generator.hpp` | ✅ (c3b1dac)・NPU L/bigG・guidance 7.5・20step・PNG 1024²・test 35/35 緑。**prompt 供給元は将来 Phase 4 A の自作 LM に差し替える予定** (現状は `req.prompt` 直入力 = LM 段未結線) |
| 2-6c | 拡散 backend プラグイン枠 `IDiffusionBackend` registry | `server/diffusion_backend.*`, `sdxl_backend.hpp`, `sd35_backend.hpp` (stub), `backend_image_generator.hpp` | ✅ (2026-07-02)・純 cpp 境界で OV/CUDA 隔離・env `DOLLAMA_BACKEND`・全 46 test 緑。**動機**: 品質天井は自作カーネルでなく拡散アーキ (重み) にあるため、prompt→RGB 境界を純 cpp interface `IDiffusionBackend` に切り出し registry (`make_backend` "sdxl"/"sd35") 化。`SDXLBackend` (OV+CUDA 隔離) + `SD35Backend` (拡張点 stub・`generate()` は throw) + `BackendImageGenerator` (解像度 reject / seed 決定 / 採点ログ / matting PNG 化 の共通後処理を集約)。段1 DI を `Txt2ImgGenerator` から `BackendImageGenerator` へ差し替え (`cli_generate.hpp`・env `DOLLAMA_BACKEND` で選択・既定 "sdxl")。ComfyUI 的 breadth は追わず「2D キャラ生成に要るアーキだけ芯を共有して差し替える」棲み分け ([[project-output-quality-over-features]]) |
