...
| 段 | 実装物 | 難易度 | 状態 |
|---|---|---|---|
| L-1 | **offline merge**: LoRA を base safetensors に事前マージ (W′=W+α·BA) → 合成 checkpoint をロード。**自作 CUDA 推論は無改修**・前処理ツール 1 本で済み、単一バイナリ / 自作美学と完全両立。「画風 / OC を 1 組焼き込む」入口 (安く capability を得る) | 低 | 未着手 |
