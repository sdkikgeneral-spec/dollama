...
> ★★**合流時の前提差分 (2026-09-20・必読)**
>
> 本書は **2026-09-05 に起草**され、**そのまま 2 週間 branch 上で放置**されたのち main へ合流した。
> 起草時点の前提のうち、**G-10k の状態に関する記述はすべて古い**:
>
> - 本書は複数箇所で「**G-10k は T2c で保留中・再開点 T3**」と書くが (§10 の干渉回避 1〜3・§12 の 6)、
>   **G-10k は再開され T3〜T9 を実施のうえ陰性クローズした (2026-09-17)**。
>   削減率 +3.38% / +1.87% = 秒中立で `DOLLAMA_CONV_BATCH=1` の opt-in へ降格 (`c3ee3ca`)。
>   → **「G-14k が GPU を離すまで G-10k を再開しない」という干渉回避は、もう成立しない**
>   (G-10k は既に終わっている)。**逆向きの注意が要る**: G-14k が attention を書き換えると、
>   G-10k が残した resnet アンカー (T2c の 0.854 s) と**別ツリーの値**になる。
> - §4 (不変条件) の env 節が言う「`DOLLAMA_CONV_BATCH` は G-10k が**予約済み**」は、**現在は実在する env** である
>   (`src/kernels/conv2d.cu:479` の `getenv` / `conv_batch_enabled()` `:470`・2026-09-20 現物確認)。
>   名前を再利用してはいけないという結論は変わらない。
> - 本書 §3 / §11 が引く秒・% の出典 (G-10k T2c の nsys) は、合流後 **`docs/g10k-plan.md` §14** に
>   整理して収載した。★とくに **launch 谷 1.79% は「生成 #2」限定**で、**生成 #1 は 3.43〜3.54%**
>   (約 2 倍) である (§14-1)。**1.79% を生成一般の値として引かないこと。**
> - 起草時に出た**未決 2 件は未決のまま**: ①`bench_pair` の既存呼出が 3 本とも B=1 で
>   PL 指定の「B=2 速度ゲート」を通らない ②VAE mid attention が G-14k のスコープか。
>   経緯は `docs/logs/g10k-baseline/r1-audit-findings.md` 末尾「是正結果」。
>
> **以上は前提の注記であって、S1 以降の設計・ゲートの変更ではない** (それは PL 決裁事項)。

## ステータス (2026-09-05 起草・★上の合流時注記を先に読むこと)

**S0 のみ実施中 (本書 = S0 の成果物)。S1 以降はすべて予定。**
★**2026-09-20 合流時点でも S1 以降は未着手のまま。**
