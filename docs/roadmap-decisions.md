...
全判定軸成立)。**この時点では頭打ちは見えず「単調・頭打ちなし」と判断** (後に B-3 で訂正)。
**決裁済 (2026-06-24・ユーザー)**: **レシピ既定化を確定** — 今後の訓練 (A 実ペア増 /
...
## 完了アイテムの実装詳細 (roadmap.md から退避)

roadmap.md の各セルを状態要約に圧縮した際に落とした実装詳細・決裁帰属。

### LoRA L-1 offline merge (✅ 完了 2026-06-24・`aec741e`)

host 側 safetensors 重み演算のみ (SDXL 推論不要・開発機で完結)。**fp32 計算 → 出力 dtype に round**・
**kohya → diffusers key 写像** (base を真実源とする)・**UNet のみ** (`lora_te1`/`te2` は warn+skip)・
**Conv は 4D reshape して加算**。`W′ = W + scale·BA` (scale=strength·alpha/rank)。test_dollma_merge_lora.py
**12/12 緑** (W+scale·BA vs numpy max err 0・key 写像 golden・触れない key は bitwise 不変・safetensors
...
**見送り 2 件**: ① **ステップ削減は不採用** (最終出力が変わるため)。② **latent preview** (本番生成の
途中 decode で忠実プレビュー) **も今回は見送り** (同上)。

### 成人向け後処理: モザイク/バー修正 — 根拠 (2026-06-24 ユーザー判断で積む)

頒布前提なら事実上必須 (刑法175条・無修正不可)。matting と同じ後処理段に 2 パーツ: ① NSFW 領域検出
(アニメ向けモデルを OV グルー 1 段・**山はモデル選定**) ② 修正処理 (モザイク=領域ブロック平均 / バー=
黒白塗り・**モデル不要の純画像処理**)。粗さは**法的慣行 (画素数基準) / プラットフォーム規定 (DLsite/FANZA
等) に合わせて可変**。`OutputSpec` に `censor: none/mosaic/bar` + ブロックサイズ。**タグでの擬似修正
(`mosaic censoring` 等は vocab にあり) は位置・粗さ不定で頒布要件を満たさない**。**絶対線: 成人キャラ
のみ (未成年不可)**。

### ガヤ (群衆) 複数人出力 — 起点 B 確定 (2026-06-24 ユーザー判断)

案 B (1人ずつ生成 → ISNet 透過 PNG → 下流で重ねる) を起点に確定。案 A (instance segmentation で 1 枚の
群衆を人物分離) は B で足りなくなった場合の追加方向 (アニメ向け成熟度低・縁再仕上げ・隠れ部分 inpainting
=ハード)。一体感のある群衆を 1 枚で出しつつ分離もしたい場合のみ A。

### 遠隔 HW ノード — 補足

critic 消費者は **pipeline 化 (N+1 生成の裏で N を採点) も batch offline (F 訓練) も両対応**・レイテンシ
非律速。① 訓練/sweep 分散の npz 集約は無負荷。
