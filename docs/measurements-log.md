...
- **中4 (是正済)**: CLAUDE.md が G-8k を未完了前提のまま (残債②で S5b から宿題化されていたが未実施
  ...
  `docs/hw-accel-plan.md:136-145` (grep: `G-8k (step ループ内 cudaMalloc/cudaFree 撲滅)` ★**行番号は 3 度動いた**: `:117/126-130/145` → 2026-09-04 R-1 で `:121/130-134/152` → **2026-09-20 検算で `:127` (順序図) と `:136-145` (本文)**)・`.claude/agents/perf-profiler.md:142-143` (grep: `G-8k (cudaMalloc/cudaFree 撲滅) は S1〜S4b 全緑でクローズ済`。★旧記述 `:101-102` は腐っており現物は無関係な「5. スレッド側を疑うときは…」だった = **2026-09-20 検算で是正**) は是正済みで、
