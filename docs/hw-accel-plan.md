...
| 自作拡散 e2e (正典 CFG・warm) | default **20.93s** / `--fast` **15.88s** (20step/1024²/VAE 込)。★**2026-07-09 = pre-G-8k の実測で陳腐化・要再測** (再測は G-10k T7c・未実施。値は差し替えていない) | test_diffusion_batch2 DB2_BENCH (fast-mode-plan) |
...
- **レイテンシ局面 (単発生成の初動)**: 非 GPU 段が直列に乗るのはプロンプト前段 (LM + CLIP) と
  ...
  **陳腐化している** (再測は G-10k T7c・未実施)。**比率の桁 (GPU が律速の全部) は変わらないが、
