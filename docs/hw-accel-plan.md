...
| 自作拡散 e2e (正典 CFG・warm) | default **20.93s** / `--fast` **15.88s** (20step/1024²/VAE 込)。★**2026-07-09 = pre-G-8k の実測で陳腐化・要再測** (★**再測は未起票** — 予定していた G-10k T7c はスキップ・G-10k 陰性クローズ = §1.2 の「分母の注意」。値は差し替えていない) | test_diffusion_batch2 DB2_BENCH (fast-mode-plan) |
...
- **レイテンシ局面 (単発生成の初動)**: 非 GPU 段が直列に乗るのはプロンプト前段 (LM + CLIP) と
  ...
  ★**分母の注意 (2026-09-04)**: 下表の分母 20.93s と `--fast` 15.88s は **2026-07-09 = pre-G-8k の実測**で
  **陳腐化している**。**比率の桁 (GPU が律速の全部) は変わらないが、
  この分母を新しい見積り・合否の分母にしないこと。**
  ★**射程の明示と再測担当の是正 (2026-09-20)**: ①**この注意は下表だけでなく、本 doc 内で 20.93s /
  15.88s を分母に使うすべての箇所に掛かる** — とくに **§「残る不明点」の P-4 行** (G-6k 出荷判定の
  同条件比較) は**将来の合否の分母**としてこの値を指名しているので、注意の射程外だと読まないこと
  (2026-09-05 の監査 Y-5)。②旧記述の「再測は G-10k T7c・未実施」は**現況と食い違う**:
  T7c は T7b が陰性だったため規定どおり**スキップ**され、G-10k 自体が陰性クローズした
  (`docs/g10k-plan.md` §13 の **T7b 節**「T7c はスキップ」。★飛び先の訂正 2026-09-20: 旧記述は「T8 節」
  だったが、当該文言は T7b 節にある)。**再測の担い手は現在いない = 未起票。**
...
- **G-8k (step ループ内 cudaMalloc/cudaFree 撲滅)** は ✅ **完了・クローズ済 (S1〜S4b・2026-08-19)**。
  ...
  (起票当時の言い方では「秒数の本命は **G-10k = conv 真 batch2**」)。
  ★**追随 (2026-09-20)**: **G-10k も陰性クローズ済 (2026-09-17)** — 削減率 **+3.38% / +1.87% (悪化方向)**
  = 秒中立で `DOLLAMA_CONV_BATCH=1` の **opt-in へ降格**・既定は per-n 直列 (`c3ee3ca`。正本
  `docs/g10k-plan.md` §13 T7b / §13 後続処置)。したがって
  **GPU 側で次に残っているのは G-9k → G-5k (FP8) → G-6k** で、そこに **G-14k (attention) が計画のみ**で加わる
  (`docs/g14k-plan.md`)。**「秒数の本命は G-10k」という前提はもう成立しない** (G-10k 台帳 §13 T8 残債③)。
  ★上の順序図 (§4 冒頭のブロック図) はまだ G-10k を未完了として描いている = **未追随の残債**。
  ★**語義の注意 (2026-09-04)**: この「秒数レバーではなかった」は**起票の目的**の話であって、
  **「実測で秒が動かなかった」という意味ではない** (S4b の A/B は既定 vs `DOLLAMA_POOL=0` で **−33.4〜−34.0%**)。
  解消は `docs/fast-mode-plan.md` の「G-8k … 実装記録」冒頭の**「用語の解消」**に 1 箇所へまとめてある。
...
| P-4 | G-6k 出荷判定の同条件比較 (dollama vs diffusers を cold+OV 込みで揃える) | 現分母 warm 20.93s vs diffusers 3.8s (cold 込み) は条件不一致 (fast-mode-plan 明記)。★**さらに 20.93s 自体が pre-G-8k で陳腐化**しており (§1.2「分母の注意」)、**この値のまま出荷判定の分母にしないこと**。再測は未起票 | G-6k 側で実施 |
