...
- **レイテンシ局面 (単発生成の初動)**: 非 GPU 段が直列に乗るのはプロンプト前段 (LM + CLIP) と
  ...
  |---|---|---|---|---|
  | LLM タグ生成 (現 Qwen2-1.5B) | CPU | **~2s** | ~9% | **非 GPU 側で唯一の非誤差帯**。ただし解は「CPU を速くする」ではなく自作 LM 差し替え (下記) |
  | └ 将来: 自作タグ生成 LM | GPU | seq8 **2.43ms** (実装済・test_bitnet_gpu) | ~0% | 差し替えで LM 段は消滅同然 |
...
- **結論**: **e2e を動かすのは GPU 側 (fast-mode-plan の G 系) だけ**。非 GPU 側でレイテンシに意味を持つ
  唯一の項目は「Qwen2 ~2s → 自作 LM (GPU 2.43ms)」の**差し替え**であり、これは CPU 高速化ではなく
...
| Qwen2 → 自作 LM 差し替え (パイプライン結線) | 正典重み焼成済・結線残 | **あり (単発レイテンシ −2s・唯一の非誤差レバー)** | Phase 4 本線 (本 doc スコープ外・M4) |
