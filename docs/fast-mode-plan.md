...
- **現状**: `src/kernels/conv2d.cu:483-503` — N>1 は per-n オフセットで N==1 ヘルパを **直列 N 回** 呼ぶ (G-2k S1 のビット一致優先設計)。これが「batch2 が resnet に効かない」主因 (G-2k 注記)。
  (★行番号の併記・T8 2026-09-17: 起草時の `:483-503` は当時の値。**現物 `src/kernels/conv2d.cu:487-507` (2026-08-24 確認・HEAD `646cc66` = T4 前・`if (N > 1 && use_gemm_path…)` `:487` / `for (int n = 0; n < N; ++n)` `:491`)**。G-10k T4 以降は同ループが `if (conv_batch_enabled())` の else 側に残り、`c3ee3ca` 現物では `for (int n = 0; n < N; ++n)` = `:843` (行番号非依存の指し方は `grep -n "for (int n = 0; n < N; ++n)" src/kernels/conv2d.cu`)。)
